...
void App::renderSkeleton() {
	...
	for (auto i = 0u; i < transforms.size(); ++i) {
		...
		glColor3f(1, 0, 0); // red
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); glVertex3f(joint_world_pos.x + scale * right[0], joint_world_pos.y + scale * right[1], joint_world_pos.z + scale * right[2]);

		// ..and the y axis.. ("up")
		glColor3f(0, 1, 0); // green
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); glVertex3f(joint_world_pos.x + scale * up[0], joint_world_pos.y + scale * up[1], joint_world_pos.z + scale * up[2]);

		// ..and the z axis ("ahead").
		glColor3f(0, 0, 1); // blue
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); glVertex3f(joint_world_pos.x + scale * ahead[0], joint_world_pos.y + scale * ahead[1], joint_world_pos.z + scale * ahead[2]);
		...
		if (parent != -1) {
			...
			glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); glVertex3f(parent_world_pos.x, parent_world_pos.y, parent_world_pos.z);
		}
...
vector<Vertex> App::computeSSD(const vector<WeightedVertex>& source_vertices) {
	...
	for (const auto& sv : source_vertices) {
		...
		Vec3f partials_pos = Vec3f();
		Vec3f partial_norm = Vec3f();
		Mat4f tryal;
		for (int i = 0; i < WEIGHTS_PER_VERTEX; i++) {
			int joint = sv.joints[i];

			Vec3f temp_pos = ssd_transforms[joint] * sv.position;
			partials_pos += sv.weights[i] * temp_pos;

			Vec3f temp_norm = ssd_transforms[joint].getXYZ() * sv.normal;
			partial_norm += sv.weights[i] * temp_norm;
		}
		v.position = partials_pos;	
		v.normal = partial_norm;		
		v.color = sv.color;
